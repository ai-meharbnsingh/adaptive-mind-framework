    ...
    return ProviderProfiles(
        ...
        profiles={
            "openai": {
                ...
                "gpt-4-turbo": CostProfile(
                    input_cpm=Decimal("10.00"), output_cpm=Decimal("30.00")
                ),
                "_default": CostProfile(
                    input_cpm=Decimal("7.00"), output_cpm=Decimal("21.00")
                ),
            },
            "anthropic": {
                "claude-3-opus": CostProfile(
                    input_cpm=Decimal("15.00"), output_cpm=Decimal("75.00")
                ),
                "claude-3-sonnet": CostProfile(
                    input_cpm=Decimal("3.00"), output_cpm=Decimal("15.00")
                ),
                "claude-3-5-sonnet-20240620": CostProfile(
                    input_cpm=Decimal("3.00"), output_cpm=Decimal("15.00")
                ),
                "_default": CostProfile(
                    input_cpm=Decimal("9.00"), output_cpm=Decimal("45.00")
                ),
            },
            "google_gemini": {
                "gemini-1.5-flash-latest": CostProfile(
                    input_cpm=Decimal("0.35"), output_cpm=Decimal("1.05")
                ),
                "gemini-pro": CostProfile(
                    input_cpm=Decimal("0.25"), output_cpm=Decimal("0.50")
                ),
                "_default": CostProfile(
                    input_cpm=Decimal("0.30"), output_cpm=Decimal("0.75")
                ),
            },
        },
    )
...
def mock_bias_ledger(mocker):
    mock_ledger = mocker.patch(
        "antifragile_framework.resilience.bias_ledger.BiasLedger",
        autospec=True,
    ).return_value
...
def mock_guards(mocker):
    ...
    def guard_factory(*args, **kwargs):
        ...
        # ==============================================================================
        # CRITICAL FIX: Corrected stateless mock logic
        # ==============================================================================
        def corrected_side_effect_factory():
            """
            CORRECTED: Always returns the first (healthiest) key, mimicking real ResourceGuard behavior.
            This fixes the statefulness issue that was causing test failures.
            """
        ...
        mock_guard.get_resource.side_effect = corrected_side_effect_factory()
        mock_guard.has_healthy_resources.return_value = True
        created_guards[provider_name] = mock_guard
        return mock_guard

    mocker.patch(
        "antifragile_framework.core.failover_engine.ResourceGuard",
...
@pytest.mark.asyncio
async def test_success_on_first_provider_model_and_key(
    engine, mock_openai_adapter, mock_guards
):
    mock_openai_adapter.return_value = CompletionResponse(
        success=True, content="Success", latency_ms=100.0
    )
    messages = [ChatMessage(role="user", content="Test")]
    response = await engine.execute_request(
        model_priority_map={"openai": ["gpt-4o"]}, messages=messages
    )
    assert response.success is True
    mock_openai_adapter.assert_called_once_with(
        messages, api_key_override="key-openai-1", model="gpt-4o"
    )
    assert mock_guards["openai"].get_resource.call_count == 1


@pytest.mark.asyncio
async def test_key_rotation_on_single_model(
    engine, mock_openai_adapter, mock_bias_ledger, mock_guards
):
    # ==============================================================================
    # UPDATED: This test needs custom mock behavior to simulate key rotation
    # ==============================================================================
    @contextmanager
    def _mock_resource_context(key_id):
...
@pytest.mark.asyncio
async def test_model_failover_when_first_model_exhausts_keys(
    engine, mock_openai_adapter, mock_guards
):
    @contextmanager
    def _mock_resource_context(key_id):
    ...
    def side_effect_func():
        ...
        if call_count <= 2:
            return _mock_resource_context(
                "key-openai-" + str(call_count)
            )  # keys for model 1
        if call_count == 3:
            raise NoResourcesAvailableError(provider="openai")
        if call_count == 4:
            return _mock_resource_context("key-openai-1")  # key for model 2
        raise NoResourcesAvailableError(provider="openai")
    ...
    mock_openai_adapter.side_effect = [
        ...
        CompletionResponse(
            success=True, content="Success on model 2", latency_ms=150.0
        ),
    ]
    messages = [ChatMessage(role="user", content="Test")]
    response = await engine.execute_request(
...
@pytest.mark.asyncio
async def test_model_specific_error_skips_key_rotation_and_tries_next_model(
    engine, mock_openai_adapter, mock_guards
):
    # ==============================================================================
    # CRITICAL: This test now uses the corrected mock behavior (always first key)
    # ==============================================================================
    error = openai.NotFoundError(
        "The model `gpt-4o` does not exist", response=Mock(), body=None
    )
    mock_openai_adapter.side_effect = [
        error,
        CompletionResponse(success=True, content="Success", latency_ms=100.0),
    ...
    calls = [
        call(messages, api_key_override="key-openai-1", model="gpt-4o"),
        call(
            messages, api_key_override="key-openai-1", model="gpt-4-turbo"
        ),  # FIXED: Same key for both calls
    ]
    mock_openai_adapter.assert_has_calls(calls)


@pytest.mark.asyncio
async def test_provider_failover_on_all_models_failing_due_to_cooldown(
    engine, mock_openai_adapter, mock_anthropic_adapter, mock_guards
):
    # ==============================================================================
    # UPDATED: Custom mock behavior for this test to simulate key exhaustion
    # ==============================================================================
    @contextmanager
    def _mock_resource_context(key_id):
        yield MagicMock(value=key_id)
    ...
    def side_effect_func():
        ...
        call_count += 1
        if call_count <= 2:
            return _mock_resource_context("key-openai-" + str(call_count))
        else:
            raise NoResourcesAvailableError(provider="openai")
...
@pytest.mark.asyncio
async def test_provider_failover_on_circuit_breaker_trip(
    engine, mock_openai_adapter, mock_anthropic_adapter, mock_guards
):
...
@pytest.mark.asyncio
async def test_all_providers_and_models_fail_raises_final_error(
    engine,
    mock_openai_adapter,
    ...
    # ==============================================================================
    # UPDATED: Custom mock behavior for comprehensive failure test
    # ==============================================================================
    @contextmanager
    def _openai_resource_context(key_id):
        yield MagicMock(value=key_id)

    @contextmanager
    def _anthropic_resource_context(key_id):
        yield MagicMock(value=key_id)

    @contextmanager
    def _gemini_resource_context(key_id):
        yield MagicMock(value=key_id)

    openai_call_count = 0

    def openai_side_effect():
    ...
    def anthropic_side_effect():
        nonlocal anthropic_call_count
        anthropic_call_count += 1
        if anthropic_call_count <= 1:
            return _anthropic_resource_context("key-claude-1")
        else:
            raise NoResourcesAvailableError(provider="anthropic")

    gemini_call_count = 0

    def gemini_side_effect():
        nonlocal gemini_call_count
        gemini_call_count += 1
        if gemini_call_count <= 1:
            return _gemini_resource_context("key-gemini-1")
        else:
            raise NoResourcesAvailableError(provider="google_gemini")

    mock_guards["openai"].get_resource.side_effect = openai_side_effect
...
@pytest.mark.asyncio
async def test_content_policy_error_triggers_successful_mitigation(
    engine_with_rewriter,
    mock_openai_adapter,
    mock_rewriter,
    mocker,
    ...
    mocker.patch.object(
        engine.error_parser,
        "classify_error",
        return_value=mock_content_policy_details,
    )
    mock_openai_adapter.side_effect = [
    ...
    mock_rewriter.rephrase_for_policy_compliance.return_value = [
        ChatMessage(role="user", content="safe")
    ]
...
@pytest.mark.asyncio
async def test_mitigation_fails_if_rewriter_fails(
    engine_with_rewriter,
    mock_openai_adapter,
    mock_rewriter,
    mocker,
    ...
    mock_openai_adapter.side_effect = openai.BadRequestError(
        "policy",
        response=Mock(),
        body={"error": {"code": "content_policy_violation"}},
    )
...
@pytest.mark.asyncio
async def test_mitigation_fails_if_rephrased_prompt_also_fails(
    engine_with_rewriter,
    mock_openai_adapter,
    mock_rewriter,
    mocker,
...
@pytest.mark.asyncio
async def test_content_policy_error_fails_fast_if_no_rewriter(
    engine, mock_openai_adapter, mocker, mock_content_policy_details
):
    mocker.patch.object(
        engine.error_parser,
        "classify_error",
...
class TestDynamicProviderRanking:
    @pytest.mark.asyncio
    async def test_dynamic_ranking_overrides_static_order(
        self,
        engine,
        mock_ranking_engine,
        mock_openai_adapter,
        ...
        mock_ranking_engine.get_ranked_providers.return_value = [
            "anthropic",
            "openai",
        ]
        mock_anthropic_adapter.return_value = CompletionResponse(
            success=True, content="Success from Anthropic", latency_ms=100.0
        )
        await engine.execute_request(
            model_priority_map={
                "openai": ["gpt-4o"],
...
class TestUserIntentAndCostManagement:
    ...
    @pytest.mark.asyncio
    async def test_preferred_provider_circuit_open_falls_back(
        self,
        engine,
        mock_openai_adapter,
        mock_anthropic_adapter,
        ...
        mock_anthropic_adapter.return_value = self._create_success_response(
            provider_name="anthropic", model_used="claude-3-sonnet"
        )
        await engine.execute_request(
            model_priority_map={
                "openai": ["gpt-4o"],
    ...
    @pytest.mark.asyncio
    async def test_preferred_provider_all_keys_unhealthy_falls_back(
        self,
        engine,
        mock_openai_adapter,
        mock_anthropic_adapter,
    ...
    @pytest.mark.asyncio
    async def test_preferred_provider_all_models_exceed_cost_cap_initial_check_falls_back(
        self,
        engine,
        mock_openai_adapter,
        mock_gemini_adapter,
        ...
        kwargs = mock_bias_ledger.log_request_lifecycle.call_args.kwargs
        assert (
            kwargs["failover_reason"]
            == "ALL_PREFERRED_MODELS_EXCEED_COST_CAP_INITIAL_CHECK:openai"
        )
    ...
    @pytest.mark.asyncio
    async def test_all_providers_fail_due_to_cost_cap(
        self,
        engine,
        mock_openai_adapter,
        mock_anthropic_adapter,
    ...
    @pytest.mark.asyncio
    async def test_preferred_unhealthy_and_fallback_cost_skips_success(
        self,
        engine,
        mock_anthropic_adapter,
        mock_gemini_adapter,
