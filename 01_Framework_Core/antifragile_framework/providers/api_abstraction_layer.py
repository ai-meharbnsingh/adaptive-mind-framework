...
class ChatMessage(BaseModel):
    ...
    def check_role_and_content_not_empty(self) -> 'ChatMessage':
        if not self.role or not self.role.strip(): raise ValueError("Message 'role' cannot be empty.")
        if not self.content or not self.content.strip(): raise ValueError(
...
class TokenUsage(BaseModel):
    ...
    output_tokens: int = Field(..., description="Number of tokens in the generated completion.")
...
class CompletionResponse(BaseModel):
    ...
    usage: Optional[TokenUsage] = Field(None, description="Token usage information for the request, if available.")
...
class LLMProvider(ABC):
    def __init__(self, config: Dict[str, Any]):
        if not isinstance(config, dict): raise TypeError("Provider configuration must be a dictionary.")
        self.config = config
        self.provider_name = self.get_provider_name()

    def _prepare_provider_messages(self, messages: List[ChatMessage]) -> Tuple[str, List[Dict[str, Any]]]:
        system_prompt, other_messages = "", []
        if not messages: return system_prompt, other_messages
