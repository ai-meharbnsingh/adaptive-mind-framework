...
from antifragile_framework.config.config_loader import load_provider_profiles, load_resilience_config
...
def malformed_provider_profiles_data():
    ...
    return {
        "schema_version": "1.0",
        "last_updated_utc": "2025-08-19T10:00:00Z",
        "profiles": {"openai": {"gpt-4o": {"input_cpm": "five", "output_cpm": "15.00"}}}
    }
...
def test_load_provider_profiles_schema_validation_error(invalid_provider_profiles_data):
    ...
    with patch("builtins.open", mock_open(read_data=mock_json_content)):
        with patch("os.path.exists", return_value=True):
            with pytest.raises(ValueError, match="Provider profiles configuration is invalid"):
                load_provider_profiles()


def test_load_provider_profiles_data_type_error(malformed_provider_profiles_data):
...
def test_load_resilience_config_success(valid_resilience_data):
    ...
    with patch("builtins.open", mock_open(read_data=mock_yaml_content)):
        with patch("os.path.exists", return_value=True):
            ...
            assert config['resilience_score_penalties']['mitigated_success_penalty'] == 0.1
