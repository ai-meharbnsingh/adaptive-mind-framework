...
class OpenAIProvider(LLMProvider):
    def __init__(self, config: Dict[str, Any]):
        ...
        try:
            self.client = AsyncOpenAI(api_key=self.primary_key, max_retries=self.max_retries)
            log.info(
    ...
    async def agenerate_completion(
        ...
        if api_key_override and api_key_override != self.primary_key:
            try:
                client_to_use = AsyncOpenAI(api_key=api_key_override, max_retries=self.max_retries)
        ...
        if system_prompt:
            user_assistant_messages.insert(0, {"role": "system", "content": system_prompt})
        ...
        try:
            ...
            response = await client_to_use.chat.completions.create(
                model=model_to_use, messages=user_assistant_messages, temperature=temperature,
                max_tokens=max_tokens, **kwargs.get('provider_specific_params', {})
            )
            ...
            return CompletionResponse(success=True, content=content, model_used=response.model, usage=usage,
                                      latency_ms=framework_latency, error_message=None,
                                      raw_response=response.model_dump(),
                                      metadata=metadata)
