...
from antifragile_framework.providers.api_abstraction_layer import CompletionResponse, ChatMessage
...
class TestE2EHappyPathAndFailover:
    @pytest.mark.asyncio
    async def test_happy_path_successful_request(self, async_client: AsyncClient, mocker):
        ...
        mock_agenerate.return_value = CompletionResponse(success=True, content="This is a successful test response.",
                                                         model_used="gpt-4o", latency_ms=123.45)
        request_payload = {"model_priority_map": {"openai": ["gpt-4o"]},
    ...
    @pytest.mark.asyncio
    async def test_resilience_succeeds_on_last_available_provider(self, async_client: AsyncClient, mocker):
        mocker.patch(
            'antifragile_framework.providers.provider_adapters.openai_adapter.OpenAIProvider.agenerate_completion',
            new_callable=AsyncMock, side_effect=openai.APIError("OpenAI is down", request=mocker.Mock(), body=None))
        mocker.patch(
            'antifragile_framework.providers.provider_adapters.claude_adapter.ClaudeProvider.agenerate_completion',
            new_callable=AsyncMock,
            side_effect=anthropic.APIError("Anthropic is down", request=mocker.Mock(), body=None))
        mock_gemini = mocker.patch(
            ...
            new_callable=AsyncMock, return_value=CompletionResponse(success=True, content="Success from Gemini!",
                                                                    model_used="gemini-1.5-flash-latest",
                                                                    latency_ms=300.0,
                                                                    metadata={'provider_name': 'google_gemini'}))
        request_payload = {"model_priority_map": {"openai": ["gpt-4o"], "anthropic": ["claude-3-opus-20240229"],
...
class TestE2ECostCapping:
    @pytest.mark.parametrize("scenario", [
        {"id": "skip_one_and_succeed", "max_cost": 0.01, "expected_status": 200,
         "expected_content": "Success from Gemini!", "openai_should_be_called": False},
        {"id": "skip_all_and_fail", "max_cost": 0.0001, "expected_status": 503, "expected_content": None,
         "openai_should_be_called": False},
        {"id": "no_skipping", "max_cost": 999.0, "expected_status": 200, "expected_content": "Success from OpenAI!",
         "openai_should_be_called": True},
    ])
    @pytest.mark.asyncio
    async def test_cost_capping_scenarios(self, scenario, async_client: AsyncClient, mocker, caplog,
        ...
        test_profiles.profiles['anthropic']['claude-3-opus-20240229'].input_cpm = Decimal("100000.00")
        app.state.failover_engine.provider_profiles = test_profiles

        mock_openai = mocker.patch(
            'antifragile_framework.providers.provider_adapters.openai_adapter.OpenAIProvider.agenerate_completion',
            new_callable=AsyncMock,
            return_value=CompletionResponse(success=True, content="Success from OpenAI!", model_used="gpt-4o",
                                            latency_ms=100.0))
        mocker.patch(
            'antifragile_framework.providers.provider_adapters.claude_adapter.ClaudeProvider.agenerate_completion',
            new_callable=AsyncMock, return_value=CompletionResponse(success=True, content="Success from Anthropic!",
        ...
        request_payload = {
            "model_priority_map": {"openai": ["gpt-4o"], "google_gemini": ["gemini-1.5-flash-latest"]},
...
class TestE2EContentPolicy:
    @pytest.mark.asyncio
    async def test_content_policy_mitigation_and_retry(self, async_client: AsyncClient, mocker):
        # Arrange
        ...
        realistic_error = openai.BadRequestError("Your request was rejected as a result of our safety system.",
        ...
        mock_provider = mocker.patch(
            ...
            new_callable=AsyncMock, side_effect=[
                realistic_error,
                CompletionResponse(success=True, content="Success after rephrasing", model_used="gpt-4o",
                                   latency_ms=100.0)
            ])

        mock_rewriter = MagicMock(spec=PromptRewriter)
        mock_rewriter.rephrase_for_policy_compliance = AsyncMock(return_value=safe_messages)
        ...
        mock_rewriter.rephrase_for_policy_compliance.assert_called_once_with(original_messages)
