...
class TestLearningEngine(unittest.TestCase):
    ...
    def _create_mock_raw_event(self, bias_ledger_entry_dict: Dict[str, Any]) -> Dict[str, Any]:
        return {
            "timestamp_utc": datetime.fromisoformat(bias_ledger_entry_dict['timestamp_utc']),
    ...
    def test_get_raw_bias_ledger_entries_valid_data(self):
        mock_entry_data = [
            BiasLedgerEntry(
                request_id="req1", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="h1",
                initial_prompt_preview="p1", outcome="SUCCESS", total_latency_ms=100.0, total_api_calls=1,
                resilience_events=[], mitigation_attempted=False,
                final_provider="openai", final_model="gpt-4o",
            ...
            BiasLedgerEntry(
                request_id="req2", timestamp_utc=(self.start_time + timedelta(hours=1)).isoformat(),
                initial_prompt_hash="h2", initial_prompt_preview="p2", outcome="FAILURE", total_latency_ms=200.0,
                total_api_calls=1, resilience_events=[], mitigation_attempted=False,
                final_provider="anthropic", final_model="claude-3",
                schema_version=4, # UPDATED
                initial_selection_mode="VALUE_DRIVEN", # NEW
                preferred_provider_requested=None, # NEW
                failover_reason=None, # NEW
        ...
        entries = list(self.learning_engine.get_raw_bias_ledger_entries(self.start_time, self.end_time))
    ...
    def test_get_raw_bias_ledger_entries_malformed_data_skipped(self, mock_log):
        malformed_entry_data = {
            "request_id": "malformed_req", "timestamp_utc": self.start_time.isoformat(),
            "initial_prompt_preview": "preview", "outcome": "SUCCESS", "total_latency_ms": 100.0,
            "total_api_calls": 1, "resilience_events": [], "mitigation_attempted": False,
            "final_provider": "openai", "final_model": "gpt-4o", "schema_version": 2 # This is purposefully old/malformed
        }
        valid_entry_data = BiasLedgerEntry(
            request_id="valid_req", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="hV",
            initial_prompt_preview="pV", outcome="SUCCESS", total_latency_ms=100.0, total_api_calls=1,
            resilience_events=[], mitigation_attempted=False,
            final_provider="openai", final_model="gpt-4o",
            schema_version=4, # UPDATED
            initial_selection_mode="VALUE_DRIVEN", # NEW
        ...
        self.mock_db_interface.query_events_generator.return_value = (
            self._create_mock_raw_event(malformed_entry_data), self._create_mock_raw_event(valid_entry_data)
        )
        entries = list(self.learning_engine.get_raw_bias_ledger_entries(self.start_time, self.end_time))
        self.assertEqual(len(entries), 1)
        self.assertEqual(entries[0].request_id, "valid_req")
        mock_log.warning.assert_called_once()
        self.assertIn("Skipping malformed BiasLedgerEntry (ValidationError)", mock_log.warning.call_args[0][0])
    ...
    def test_get_raw_bias_ledger_entries_different_schema_version(self, mock_log):
        ...
        mock_entry_v2_data = BiasLedgerEntry(
            request_id="req_v2", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="h1",
            initial_prompt_preview="p1",
            outcome="SUCCESS", total_latency_ms=100.0, total_api_calls=1, resilience_events=[],
            mitigation_attempted=False,
            final_provider="openai", final_model="gpt-4o",
            schema_version=2, # This is specifically testing an older schema version if it's still parsable
            initial_selection_mode="VALUE_DRIVEN", # NEW: must be included for current model validation
            preferred_provider_requested=None, # NEW
            failover_reason=None, # NEW
            cost_cap_enforced=False, # NEW
            cost_cap_skip_reason=None # NEW
        ).model_dump()
        self.mock_db_interface.query_events_generator.return_value = (self._create_mock_raw_event(mock_entry_v2_data),)
        entries = list(self.learning_engine.get_raw_bias_ledger_entries(self.start_time, self.end_time))
        self.assertEqual(len(entries), 1)
    ...
    def test_analyze_provider_performance_basic_success(self):
        ...
        entry2_data = BiasLedgerEntry(
            request_id="req2", timestamp_utc=(self.start_time + timedelta(minutes=1)).isoformat(),
            initial_prompt_hash="h2", initial_prompt_preview="p2",
            outcome="SUCCESS", total_latency_ms=150.0, total_api_calls=1, resilience_events=[],
            mitigation_attempted=False,
            final_provider="openai", final_model="gpt-4o",
            schema_version=4, # UPDATED
            initial_selection_mode="VALUE_DRIVEN", # NEW
        ...
        self.mock_db_interface.query_events_generator.return_value = (
        self._create_mock_raw_event(entry1_data), self._create_mock_raw_event(entry2_data))
        analyses = self.learning_engine.analyze_provider_performance(self.start_time, self.end_time)
    ...
    def test_analyze_provider_performance_mixed_outcomes_and_metrics(self):
        ...
        entry1 = BiasLedgerEntry(request_id="req1", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="h1",
                                 initial_prompt_preview="p1", outcome="SUCCESS", total_latency_ms=100.0,
                                 total_api_calls=1, resilience_events=[], mitigation_attempted=False,
                                 final_provider="openai", final_model="gpt-4o", resilience_score=0.9,
                                 schema_version=4, # UPDATED
                                 initial_selection_mode="VALUE_DRIVEN", # NEW
                                 preferred_provider_requested=None, # NEW
                                 failover_reason=None, # NEW
        ...
        entry2 = BiasLedgerEntry(request_id="req2", timestamp_utc=(self.start_time + timedelta(minutes=1)).isoformat(),
                                 initial_prompt_hash="h2", initial_prompt_preview="p2", outcome="FAILURE",
                                 total_latency_ms=50.0, total_api_calls=1, resilience_events=[
                {"event_type": event_topics.API_CALL_FAILURE, "payload": {"error_type": "rate_limit"}}],
                                 mitigation_attempted=False, final_provider="openai", final_model="gpt-4o",
        ...
        entry3 = BiasLedgerEntry(request_id="req3", timestamp_utc=(self.start_time + timedelta(minutes=2)).isoformat(),
                                 initial_prompt_hash="h3", initial_prompt_preview="p3", outcome="MITIGATED_SUCCESS",
                                 total_latency_ms=300.0, total_api_calls=2,
                                 resilience_events=[{"event_type": event_topics.PROVIDER_FAILOVER}],
                                 mitigation_attempted=True, mitigation_succeeded=True, final_provider="anthropic",
                                 final_model="claude-3", resilience_score=0.7,
                                 schema_version=4, # UPDATED
                                 initial_selection_mode="VALUE_DRIVEN", # NEW
                                 preferred_provider_requested=None, # NEW
        ...
        entry4 = BiasLedgerEntry(request_id="req4", timestamp_utc=(self.start_time + timedelta(minutes=3)).isoformat(),
                                 initial_prompt_hash="h4", initial_prompt_preview="p4", outcome="SUCCESS",
                                 total_latency_ms=250.0, total_api_calls=2,
                                 resilience_events=[{"event_type": event_topics.CIRCUIT_TRIPPED},
                                                    {"event_type": event_topics.MODEL_FAILOVER}],
                                 mitigation_attempted=False, final_provider="google_gemini", final_model="gemini-pro",
        ...
        self.mock_db_interface.query_events_generator.return_value = (self._create_mock_raw_event(e) for e in
        ...
        google_analysis = next(a for a in analyses if a.provider_name == "google_gemini")
    ...
    def test_analyze_provider_performance_zero_total_requests(self):
        ...
        entry_failure_only = BiasLedgerEntry(
            request_id="req_fail_only", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="hF",
            initial_prompt_preview="pF", outcome="FAILURE", total_latency_ms=0.0, total_api_calls=1,
            resilience_events=[], mitigation_attempted=False,
            final_provider="test_provider", final_model="test_model", resilience_score=0.2,
            schema_version=4, # UPDATED
            initial_selection_mode="VALUE_DRIVEN", # NEW
            preferred_provider_requested=None, # NEW
        ...
        self.mock_db_interface.query_events_generator.return_value = (self._create_mock_raw_event(entry_failure_only),)
        analyses = self.learning_engine.analyze_provider_performance(self.start_time, self.end_time)
        self.assertEqual(len(analyses), 1)
        analysis = analyses[0]
    ...
    def test_analyze_provider_performance_no_resilience_score(self):
        entry_no_score = BiasLedgerEntry(
            request_id="req_no_score", timestamp_utc=self.start_time.isoformat(), initial_prompt_hash="hns",
            initial_prompt_preview="pns",
            outcome="SUCCESS", total_latency_ms=100.0, total_api_calls=1, resilience_events=[],
            mitigation_attempted=False,
            final_provider="openai", final_model="gpt-4o", resilience_score=None,
            schema_version=4, # UPDATED
            initial_selection_mode="VALUE_DRIVEN", # NEW
            preferred_provider_requested=None, # NEW
            failover_reason=None, # NEW
        ...
        self.mock_db_interface.query_events_generator.return_value = (self._create_mock_raw_event(entry_no_score),)
        analyses = self.learning_engine.analyze_provider_performance(self.start_time, self.end_time)
        self.assertEqual(len(analyses), 1)
