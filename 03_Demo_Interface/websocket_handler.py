...
try:
    ...
    logging.warning("RealTimeMetricsCollector not available for WebSocketManager demo. Will use local mocks.")
...
class WebSocketManager:
    ...
    def __init__(self):
        ...
        self.message_queue_per_client: Dict[Any, asyncio.Queue] = {}  # Queue for each client
    ...
    async def connect(self, websocket: Any):
        ...
        logger.info(f"New WebSocket connected. Total active: {len(self.active_connections)}")
    ...
    async def send_personal_message(self, message: Dict[str, Any], websocket: Any):
        ...
        try:
            ...
            logger.error(f"Failed to send personal message to WebSocket: {e}", exc_info=True)
    ...
    async def broadcast(self, message: Dict[str, Any]):
        ...
        for connection in list(self.active_connections):  # Iterate over a copy
            try:
                ...
                logger.warning(f"Client queue full for {connection.client}. Disconnecting due to backlog.")
                disconnected_clients.append(connection)
            except KeyError:  # Client might have disconnected between list() and here
                pass  # Already handled by disconnect
            except Exception as e:
                logger.error(f"Error queuing message for client {connection.client}: {e}", exc_info=True)
                disconnected_clients.append(connection)
        ...
        for connection in self.active_connections:
            if not hasattr(connection, '_websocket_listener_task'):  # Check if task exists
    ...
    async def _websocket_listener(self, websocket: Any):
        ...
        if not queue:
            logger.warning(f"No queue found for websocket {websocket.client}. Listener exiting.")
            return
        ...
        try:
            ...
            logger.error(f"WebSocket listener task failed for {websocket.client}: {e}", exc_info=True)
            self.disconnect(websocket)
        finally:
            if hasattr(websocket, '_websocket_listener_task'):
                del websocket._websocket_listener_task  # Clean up task attribute

    async def broadcast_dashboard_updates_periodically(self, metrics_collector: RealTimeMetricsCollector,
        ...
        logger.info(f"Starting periodic dashboard update broadcast every {interval_seconds} seconds.")
        ...
        async def _periodic_task():
            while True:
                try:
                    ...
                    await self.broadcast({"type": "dashboard_update", "data": live_metrics_data,
                                          "timestamp": datetime.now(timezone.utc).isoformat()})
                except Exception as e:
                    logger.error(f"Error during periodic dashboard update broadcast: {e}", exc_info=True)
    ...
    async def shutdown(self):
        ...
        for websocket in list(self.active_connections):
            if websocket in self.message_queue_per_client:
                await self.message_queue_per_client[websocket].put(None)  # Sentinel value

        # Wait for listener tasks to complete (with a timeout)
        await asyncio.gather(*[
            task for task in [
                getattr(conn, '_websocket_listener_task', None) for conn in self.active_connections
            ] if task is not None and not task.done()
        ], return_exceptions=True)  # Gather with return_exceptions to not fail on individual task errors

        for websocket in list(self.active_connections):
            try:
                await websocket.close()
            except Exception as e:
                logger.warning(f"Error closing WebSocket connection during shutdown: {e}")
            self.disconnect(websocket)
...
if __name__ == "__main__":
    ...
    async def main():
        ...
        mock_metrics_collector = RealTimeMetricsCollector()  # This class now includes a mock for itself if DB is unavailable
        ...
        class MockWebSocket:
            ...
            async def accept(self): print(f"MockWebSocket {self.client} accepted.")

            async def send_text(self, message):
                self._messages_sent.append(message)
                print(f"MockWebSocket {self.client} received: {message[:100]}...")  # Print first 100 chars
            ...
            async def close(self): print(f"MockWebSocket {self.client} closed.")
        ...
        manager.broadcast_dashboard_updates_periodically(mock_metrics_collector, interval_seconds=2)
        ...
        await manager.send_personal_message({"type": "custom_alert", "message": "High anomaly detected!"}, mock_ws1)
