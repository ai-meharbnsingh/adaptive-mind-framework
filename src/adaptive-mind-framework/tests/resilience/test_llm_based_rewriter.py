...
from antifragile_framework.resilience.llm_based_rewriter import LLMBasedRewriter
...
async def test_rephrase_success(mock_llm_client):
    ...
    mock_llm_client.agenerate_completion.return_value = CompletionResponse(
        success=True, content=rephrased_text, provider_name="mock_provider",
...
async def test_rephrase_api_call_fails(mock_llm_client):
    mock_llm_client.agenerate_completion.return_value = CompletionResponse(
        success=False, error_message="API limit reached",
        provider_name="mock_provider", model="test-model", latency_ms=50.0
    )
