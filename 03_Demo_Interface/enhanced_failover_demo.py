...
class EnhancedFailoverDemo:
    ...
    async def execute_enhanced_failover_demo(
        self,
        session_id: str,
        initial_prompt: str,
        ...
        try:
            ...
            scenario = self.demo_scenarios.get(
                failover_scenario_key, self.demo_scenarios["primary_api_down"]
            )
            ...
            context_backup = await self._backup_context(
                current_conversation, initial_prompt
            )
            ...
            final_response = await self._generate_final_response(
                initial_prompt,
                failover_sequence_result["successful_provider"],
                context_validation,
            )
            ...
            self.logger.info(
                f"✅ Enhanced failover demo completed successfully in {total_demo_time_ms}ms"
            )
            return demo_results
    ...
    async def _simulate_initial_failure(
        self, provider: str, scenario: Dict[str, Any], prompt: str
    ) -> FailoverStep:
        ...
        self.logger.warning(
            f"🔴 Simulated failure: {provider} - {scenario['trigger'].value}"
        )
        return step

    async def _backup_context(
        self, conversation: List[Dict[str, str]], current_prompt: str
    ) -> Dict[str, Any]:
    ...
    async def _execute_failover_sequence(
        ...
        existing_steps: List[FailoverStep],
    ) -> Dict[str, Any]:
            ...
            step = FailoverStep(
                ...
                details={
                    ...
                    "response_quality_score": (
                        random.uniform(0.85, 0.98) if is_successful else 0.0
                    ),
    ...
    async def _validate_context_preservation(
        self, original_context: Dict[str, Any], final_context: Dict[str, Any]
    ) -> ContextValidation:
        ...
        for element in self.context_elements:
            if element in original_context.get(
                "context_elements", {}
            ) and element in final_context.get("context_elements", {}):
        ...
        self.logger.info(
            f"🔍 Context preservation: {preservation_score:.1f}% ({preserved_count}/{total_elements})"
        )
    ...
    def _calculate_performance_impact(
        self, expected_ms: float, actual_ms: float
    ) -> str:
    ...
    async def _generate_fallback_demo_result(
        self, session_id: str, prompt: str, error: str
    ) -> Dict[str, Any]:
    ...
    async def simulate_provider_outage(
        ...
        self.logger.warning(
            f"🔴 Simulated outage started for {provider} ({duration_minutes}min)"
        )
