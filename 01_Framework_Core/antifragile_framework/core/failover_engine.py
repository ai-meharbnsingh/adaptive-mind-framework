...
from .exceptions import AllProviderKeysFailedError, NoResourcesAvailableError, AllProvidersFailedError, \
    ContentPolicyError, RewriteFailedError
...
# ==============================================================================
from antifragile_framework.providers.provider_registry import ProviderRegistry, get_default_provider_registry
...
DEFAULT_MAX_OUTPUT_TOKEN_ESTIMATE = 2048  # A conservative estimate for cost capping if user doesn't specify max_tokens
...
class FailoverEngine:
    def __init__(self, provider_configs: Dict[str, Dict[str, Any]],
        ...
        for name, config in provider_configs.items():
            ...
            if not api_keys: log.warning(f"No API keys for provider '{name}'. Skipping."); continue

            provider_instance_config = config.copy()
            if 'api_key' not in provider_instance_config: provider_instance_config['api_key'] = api_keys[0]
    ...
    def _load_and_validate_penalties(self):
        ...
        expected_penalties = ["base_successful_penalty", "mitigated_success_penalty", "api_call_failure_penalty",
                              "api_key_rotation_penalty", "model_failover_penalty", "provider_failover_penalty",
                              "circuit_tripped_penalty", "all_providers_failed_penalty"]
        for penalty_key in expected_penalties:
            penalty_value = penalties_config.get(penalty_key)
            if not isinstance(penalty_value, (int, float)): raise ValueError(
                f"Resilience score penalty '{penalty_key}' must be a number. Found: {penalty_value}")
            if not (0.0 <= penalty_value <= 1.0): raise ValueError(
    ...
    def _record_lifecycle_event(self, context: RequestContext, event_name: str, severity: str,
                                payload_data: Dict[str, Any]):
        timestamp = datetime.now(timezone.utc).isoformat()
        context.lifecycle_events.append({"timestamp": timestamp, "event_name": event_name, **payload_data})
        event_schema = UniversalEventSchema(event_type=event_name, event_source=self.__class__.__name__,
    ...
    def _calculate_resilience_score(self, context: RequestContext, final_outcome: str) -> float:
        score = 1.0
        if final_outcome == "MITIGATED_SUCCESS": score -= self.resilience_score_penalties.get(
            "mitigated_success_penalty", 0.0)
        for event in context.lifecycle_events:
            event_type = event.get("event_type")
            if event_type == event_topics.API_CALL_FAILURE:
                score -= self.resilience_score_penalties.get("api_call_failure_penalty", 0.0)
            elif event_type == event_topics.API_KEY_ROTATION:
                score -= self.resilience_score_penalties.get("api_key_rotation_penalty", 0.0)
            elif event_type == event_topics.MODEL_FAILOVER:
                score -= self.resilience_score_penalties.get("model_failover_penalty", 0.0)
            elif event_type == event_topics.PROVIDER_FAILOVER:
                score -= self.resilience_score_penalties.get("provider_failover_penalty", 0.0)
            elif event_type == event_topics.CIRCUIT_TRIPPED:
                score -= self.resilience_score_penalties.get("circuit_tripped_penalty", 0.0)
            elif event_type == event_topics.ALL_PROVIDERS_FAILED:
                score -= self.resilience_score_penalties.get("all_providers_failed_penalty", 0.0)
            elif event_type == event_topics.PROMPT_HUMANIZATION_ATTEMPT:
                score -= self.resilience_score_penalties.get("mitigated_success_penalty", 0.0)
    ...
    def _estimate_call_cost(self, provider_name: str, model_name: str,
                            input_tokens: int, output_tokens_estimate: int) -> Optional[Decimal]:
        ...
        try:
            ...
            if not provider_data:
                log.warning(f"No cost data for provider '{provider_name}'. Cannot estimate cost.")
                return None
            cost_profile: Optional[CostProfile] = provider_data.get(model_name)
            if not cost_profile:
                cost_profile = provider_data.get("_default")
                if cost_profile:
                    log.debug(f"Cost profile for '{provider_name}/{model_name}' not found. Using provider default.")
                else:
                    log.warning(
                        f"No cost profile or default found for provider '{provider_name}'. Cannot estimate cost.")
                    return None
            input_cost = (Decimal(input_tokens) * cost_profile.input_cpm) / Decimal('1000000')
            output_cost = (Decimal(output_tokens_estimate) * cost_profile.output_cpm) / Decimal('1000000')
            total_cost = (input_cost + output_cost).quantize(Decimal('0.000001'), rounding=ROUND_HALF_UP)
            return total_cost
        except Exception as e:
            log.error(f"Error calculating estimated cost for {provider_name}/{model_name}: {e}", exc_info=True)
            return None

    def _get_dynamic_provider_priority(self, model_priority_map: Dict[str, List[str]]) -> List[str]:
        static_providers = list(model_priority_map.keys())
        if not self.provider_ranking_engine:
            log.warning("ProviderRankingEngine not configured. Using static provider order from request.")
            return [p for p in static_providers if self._is_provider_healthy_for_dynamic_selection(p)]

        ranked_providers: List[str] = self.provider_ranking_engine.get_ranked_providers()
        if not ranked_providers:
            log.info("No provider rankings available yet. Using static provider order from request.")
            return [p for p in static_providers if self._is_provider_healthy_for_dynamic_selection(p)]

        static_providers_set = {p.lower() for p in static_providers}
        filtered_ranked_providers = [p for p in ranked_providers if p.lower() in static_providers_set]
        filtered_ranked_set = {p.lower() for p in filtered_ranked_providers}
        unranked_providers = [p for p in static_providers if p.lower() not in filtered_ranked_set]

        final_priority_unfiltered = filtered_ranked_providers + unranked_providers
        final_priority = [p for p in final_priority_unfiltered if self._is_provider_healthy_for_dynamic_selection(p)]
    ...
    def _is_provider_healthy_for_dynamic_selection(self, provider_name: str) -> bool:
        if provider_name not in self.providers:
            log.debug(f"Provider '{provider_name}' not initialized. Skipping in dynamic selection.")
            return False

        breaker = self.circuit_breakers.get_breaker(provider_name)
        if breaker.state == CircuitBreakerState.OPEN:
            log.debug(f"Provider '{provider_name}' skipped in dynamic selection: circuit is OPEN.")
            return False

        guard = self.guards.get(provider_name)
        if not guard or not guard.has_healthy_resources():
            log.debug(f"Provider '{provider_name}' skipped in dynamic selection: no healthy resources.")
            return False
    ...
    async def execute_request(
        ...
        initial_selection_mode: str = "PREFERENCE_DRIVEN" if preferred_provider else "VALUE_DRIVEN"
        failover_reason: Optional[str] = None

        provider_sequence_to_attempt: Optional[List[str]] = None

        try:
            estimated_input_tokens = self._estimate_prompt_tokens(context.initial_messages)
            output_tokens_for_estimate = kwargs.get('max_tokens', DEFAULT_MAX_OUTPUT_TOKEN_ESTIMATE)
            ...
            if preferred_provider:
                ...
                if provider_name_lower not in self.providers:
                    ...
                    try:
                        ...
                        self._record_lifecycle_event(context, event_topics.CIRCUIT_TRIPPED, "CRITICAL",
                                                     {"provider": provider_name_lower, "context": "pre-check"})
                    ...
                    if not failover_reason and max_estimated_cost_usd is not None:
                        preferred_models = model_priority_map.get(provider_name_lower, [])
                        all_too_expensive = True
                        for model in preferred_models:
                            cost = self._estimate_call_cost(provider_name_lower, model, estimated_input_tokens,
                                                            output_tokens_for_estimate)
                            if cost is None or cost <= Decimal(str(max_estimated_cost_usd)):
                        ...
                        if all_too_expensive:
                            ...
                            context.cost_cap_skip_reason = "ALL_MODELS_TOO_EXPENSIVE_FOR_PREFERENCE"
                ...
                if not failover_reason:
                    ...
                    log.warning(f"{failover_reason}. Falling back to dynamic selection.")

            if provider_sequence_to_attempt is None:
                log.info("Entering dynamic fallback mode or proceeding with value-driven selection.")
                dynamic_provider_priority = self._get_dynamic_provider_priority(model_priority_map)
                if not dynamic_provider_priority:
                    ...
                    final_exception = AllProvidersFailedError(
                        errors=["No viable dynamic providers after health/cost filtering."])
                else:
            ...
            if provider_sequence_to_attempt:
                try:
                    final_response = await self._attempt_request_sequence(
                        context, provider_sequence_to_attempt, model_priority_map, context.final_messages,
                        estimated_input_tokens, output_tokens_for_estimate, **kwargs
                    )
                    ...
                    if not self.prompt_rewriter:
                        failover_reason = failover_reason or "DYNAMIC_MODE_CONTENT_POLICY_NO_REWRITER"
                        final_exception = AllProvidersFailedError(errors=[str(e), "No prompt rewriter configured."])
                    else:
                        try:
                            ...
                            dynamic_provider_priority = self._get_dynamic_provider_priority(model_priority_map)
                            final_response = await self._attempt_request_sequence(
                                context, dynamic_provider_priority, model_priority_map, context.final_messages,
                                estimated_input_tokens, output_tokens_for_estimate, **kwargs
                            )
                            final_outcome = "MITIGATED_SUCCESS"
                        except (RewriteFailedError, ContentPolicyError, AllProvidersFailedError) as rfe:
                            failover_reason = failover_reason or "DYNAMIC_MODE_MITIGATION_FAILED"
                            final_exception = AllProvidersFailedError(
                                errors=[str(e), f"Mitigation attempt failed: {rfe}"])
            ...
            if self.bias_ledger:
                ledger_entry = self.bias_ledger.log_request_lifecycle(
                    context=context, initial_selection_mode=initial_selection_mode,
                    final_response=final_response, final_error=final_exception,
                    resilience_score=resilience_score, failover_reason=failover_reason,
                    cost_cap_enforced=context.cost_cap_enforced, cost_cap_skip_reason=context.cost_cap_skip_reason
                )
                if self.event_bus and ledger_entry:
                    payload = json.loads(ledger_entry.model_dump_json())
                    self.event_bus.publish(event_type=event_topics.LEARNING_FEEDBACK_PUBLISHED, payload=payload)

        if final_response:
            return final_response

        raise final_exception or AllProvidersFailedError(errors=["Request failed due to an unknown issue."])

    async def _attempt_request_sequence(
            self, context: RequestContext, provider_priority: List[str], model_priority_map: Dict[str, List[str]],
            messages: List[ChatMessage],
        ...
        for provider_name in provider_priority:
            ...
            if provider_name not in self.providers:
                log.warning(f"Provider '{provider_name}' not found in initialized providers. Skipping.")
                overall_errors.append(f"Provider '{provider_name}' was skipped: not initialized.")
                continue

            model_priority = model_priority_map.get(provider_name)
            if not model_priority:
                log.warning(f"No model priority list found for provider '{provider_name}'. Skipping.")
                overall_errors.append(f"Provider '{provider_name}' was skipped: no model list provided.")
                continue
            ...
            for model_name in model_priority:
                estimated_cost = self._estimate_call_cost(
                    provider_name, model_name, estimated_input_tokens, output_tokens_for_estimate
                )
                if context.max_estimated_cost_usd is not None and estimated_cost is not None:
                    if estimated_cost > Decimal(str(context.max_estimated_cost_usd)):
                        self._record_lifecycle_event(context, event_topics.MODEL_SKIPPED_DUE_TO_COST, "INFO",
            ...
            if not viable_models_for_provider:
                log.warning(f"All models for provider '{provider_name}' were skipped due to cost. Skipping provider.")
                overall_errors.append(f"Provider '{provider_name}' was skipped: no viable models after cost checks.")
                continue

            for model in viable_models_for_provider:
                if last_model: self._record_lifecycle_event(context, event_topics.MODEL_FAILOVER, "WARNING",
                                                            {"provider": provider_name, "from_model": last_model,
                                                             "to_model": model})
                last_model = model
                try:
                    response = await self._attempt_model_with_keys(context, provider_name,
                                                                   self.providers[provider_name], guard, breaker,
                                                                   model, messages, **kwargs)
                    was_half_open = breaker.state == CircuitBreakerState.HALF_OPEN
                    breaker.record_success()
                    if was_half_open: self._record_lifecycle_event(context, event_topics.CIRCUIT_RESET, "INFO",
            ...
            try:
                with guard.get_resource() as resource:
                    ...
                    try:
                        ...
                        response = await provider.agenerate_completion(messages, api_key_override=resource.value,
                                                                       **request_kwargs)
                        if response.success: return response

                        last_key_error = response.error_message or "Provider returned non-success."
                        raw_exception = response.metadata.get('raw_exception') if response.metadata else None
                        ...
                        error_details = self.error_parser.classify_error(e, provider_name)
