...
class CostOptimizer:
    ...
    async def track_request_cost(self, provider: str, model: str,
        ...
        try:
            ...
            self.logger.info(f"Tracked cost: {provider}/{model} - ${estimated_cost:.6f}")
    ...
    async def get_cost_efficiency_summary(self) -> Dict[str, Any]:
        ...
        try:
            ...
            total_cost = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
                             for entry in self.cost_history)
            ...
            return {
                ...
                "cost_efficiency_grade": self._calculate_efficiency_grade(cost_reduction_percent),
                "monthly_projected_cost": round(total_cost * 30, 2),
                "annual_projected_savings": round(
                    (benchmark_cost - avg_cost_per_request) * total_requests * 365, 2)
            }
    ...
    async def get_cost_optimization_recommendations(self,
                                                    current_mode: str = "hosted") -> List[Dict[str, Any]]:
    ...
    async def get_real_time_cost_metrics(self) -> Dict[str, Any]:
        ...
        try:
            ...
            recent_cost = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
            ...
            return {
                ...
                "efficiency_score": self._calculate_efficiency_score(avg_cost_per_request),
                "cost_alerts": self._generate_cost_alerts(recent_cost, avg_cost_per_request)
            }
    ...
    async def _analyze_cost_patterns(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # Identify patterns
            patterns = {
                "dominant_provider": max(provider_usage.items(),
                                         key=lambda x: x[1]["cost"])[0] if provider_usage else None,
                "cost_variance": self._calculate_cost_variance(),
                "peak_usage_times": self._identify_peak_times(),
                "optimization_potential": total_cost * 0.25  # Assume 25% optimization potential
            }
    ...
    def _should_recommend_rule(self, rule: Dict[str, Any],
    ...
    def _calculate_cost_variance(self) -> float:
        ...
        costs = [entry.get("estimated_cost", entry.get("total_cost", 0))
    ...
    def _calculate_cost_trend(self, entries: List[Dict[str, Any]]) -> str:
        ...
        recent_avg = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
                         for entry in entries[-10:]) / min(len(entries), 10)

        older_avg = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
                        for entry in entries[-20:-10]) / min(len(entries) - 10, 10)
    ...
    def _generate_cost_alerts(self, recent_cost: float,
                              avg_cost_per_request: float) -> List[Dict[str, str]]:
    ...
    async def get_cost_forecast(self, days: int = 30) -> Dict[str, Any]:
        ...
        try:
            ...
            daily_cost = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
                             for entry in recent_entries)
            ...
            for day in range(days):
                ...
                forecast_data.append({
                    ...
                    "confidence": max(0.6, 0.95 - (day * 0.01))  # Decreasing confidence
                })
    ...
    async def get_cost_comparison_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            for provider, total_cost in self.provider_costs.items():
                ...
                if provider_entries:
                    avg_cost = sum(entry.get("estimated_cost", 0) for entry in provider_entries) / len(provider_entries)
                    avg_tokens = sum(entry.get("total_tokens", 0) for entry in provider_entries) / len(provider_entries)
                ...
                provider_analysis[provider] = {
                    ...
                    "market_share": round((total_cost / max(sum(self.provider_costs.values()), 1)) * 100, 1)
                }

            # Find most cost-effective provider
            best_provider = min(provider_analysis.items(),
                                key=lambda x: x[1]["avg_cost_per_request"])[0] if provider_analysis else "unknown"
    ...
    async def set_cost_budget(self, daily_budget: float, alert_threshold: float = 0.8) -> Dict[str, Any]:
        ...
        try:
            ...
            current_spend = sum(entry.get("estimated_cost", entry.get("total_cost", 0))
            ...
            budget_status = {
                ...
                "budget_utilization_percent": round((current_spend / daily_budget) * 100, 1),
                "alert_threshold": alert_threshold,
                "budget_status": "on_track" if current_spend < daily_budget * alert_threshold else "alert"
            }

            self.logger.info(f"Cost budget set: ${daily_budget}/day, current: ${current_spend:.4f}")
