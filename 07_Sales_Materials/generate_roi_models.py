...
def create_executive_summary_sheet(writer):
    ...
    df_value_prop.to_excel(
        writer, sheet_name="Executive_Summary", index=False, startrow=15
    )
...
def create_industry_benchmarks_sheet(writer):
    ...
    benchmark_data = {
        ...
        "With Adaptive Mind": [
            ...
            0.015,
            0.029,
            0.047,  # Reduced API costs
            0.8,
            0.8,
            0.8,
            0.8,
            0.8,  # Consistent low failure
            15000,
            85000,
            25000,
            8000,
            45000,  # Same downtime cost base
            0.2,
            0.1,
            0.1,
            0.2,
            0.4,  # Improved response times
            5,
            8,
            12,
            4,
...
def create_cost_comparison_sheet(writer):
    ...
    df_costs["Current vs Adaptive Mind Savings"] = (
        df_costs["Current Solution (Annual)"] - df_costs["Adaptive Mind (Annual)"]
    )
    df_costs["LangChain vs Adaptive Mind Savings"] = (
        df_costs["LangChain (Annual)"] - df_costs["Adaptive Mind (Annual)"]
    )
    df_costs["Semantic Kernel vs Adaptive Mind Savings"] = (
        df_costs["Semantic Kernel (Annual)"] - df_costs["Adaptive Mind (Annual)"]
    )
    ...
    totals_data = {
        ...
        "Current vs Adaptive Mind Savings": [
            df_costs["Current vs Adaptive Mind Savings"].sum()
        ],
        "LangChain vs Adaptive Mind Savings": [
            df_costs["LangChain vs Adaptive Mind Savings"].sum()
        ],
        "Semantic Kernel vs Adaptive Mind Savings": [
            df_costs["Semantic Kernel vs Adaptive Mind Savings"].sum()
        ],
    }

    df_totals = pd.DataFrame(totals_data)
    df_totals.to_excel(
        writer, sheet_name="Cost_Comparison", index=False, startrow=len(df_costs) + 3
    )
...
def create_roi_calculator_sheet(writer):
    ...
    calculator_inputs = {
        ...
        "Default Value": [100000, 0.025, 10.0, 25000, 6, 85000, 25, 3, 8.0, 5],
        "Your Value": ["", "", "", "", "", "", "", "", "", ""],
        "Description": [
...
def create_business_impact_sheet(writer):
    ...
    impact_data = {
        ...
        "Business Value Score (1-10)": [10, 9, 10, 8, 9, 8, 7, 9, 8, 8],
    }
...
def create_implementation_timeline_sheet(writer):
    ...
    timeline_data = {
        ...
        "Duration (Days)": [3, 2, 1, 3, 5, 2, 7, 5, 7, 1],
        "Key Activities": [
    ...
    df_timeline["Start Day"] = (
        df_timeline["Cumulative Days"] - df_timeline["Duration (Days)"] + 1
    )
    df_timeline["End Day"] = df_timeline["Cumulative Days"]

    # Save updated timeline
    timeline_summary = df_timeline[["Phase", "Start Day", "End Day", "Duration (Days)"]]
    timeline_summary.to_excel(
        writer, sheet_name="Implementation_Timeline", index=False, startrow=15
    )
...
def create_financial_projections_sheet(writer):
    ...
    df_breakdown["Savings"] = (
        df_breakdown["Current Solution"] - df_breakdown["Adaptive Mind"]
    )
    df_breakdown.to_excel(
        writer, sheet_name="Financial_Projections", index=False, startrow=15
    )
...
def create_scenario_analysis_sheet(writer):
    ...
    scenario_data = {
        ...
        "Value": [
            ...
            75,
            92,
            98,  # Failure reduction
            450,
            1150,
            2250,  # Performance improvement
            50,
            75,
        ...
        "Probability (%)": [
            20,
            60,
            20,  # Annual savings probability
            20,
            60,
            20,  # ROI probability
            20,
            60,
            20,  # Payback probability
            20,
            60,
            20,  # Failure reduction probability
            20,
            60,
            20,  # Performance probability
            20,
            60,
            20,  # Maintenance probability
            20,
            60,
            20,  # Implementation probability
            20,
            60,
            20,  # Productivity probability
            20,
            60,
            20,  # Satisfaction probability
            20,
            60,
            20,  # NPV probability
        ],
    ...
    for metric in metrics:
        ...
        expected_value = sum(
            metric_data["Value"] * metric_data["Probability (%)"] / 100
        )
        expected_values[metric] = expected_value

    expected_df = pd.DataFrame(
        list(expected_values.items()), columns=["Metric", "Expected Value"]
    )
    expected_df.to_excel(
        writer, sheet_name="Scenario_Analysis", index=False, startrow=35
    )
