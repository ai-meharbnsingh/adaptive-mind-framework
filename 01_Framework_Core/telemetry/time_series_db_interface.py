...
async def main():
    class MockTimeSeriesDB(TimeSeriesDBInterface):
        ...
        List[Dict[str, Any]]:
            print(f"{self.name}: Querying {event_type} from {start_time} to {end_time} (mock data).")
            return [{"event_type": event_type, "timestamp": datetime.now().isoformat(), "payload": {"data": "mock"}} for
                    _ in range(min(limit, 2))]
        ...
        async def query_events_generator(self, event_type: str, start_time: datetime, end_time: datetime,
            ...
            for i in range(3):  # Yield 3 mock events
                yield {"event_type": event_type, "timestamp": datetime.now().isoformat(),
                       "payload": {"data": f"mock_batch_{i}"}}

        async def aggregate_events(self, event_type: str, start_time: datetime, end_time: datetime,
                                   aggregate_by: str) -> List[Dict[str, Any]]:
            print(f"{self.name}: Aggregating {event_type} by {aggregate_by} (mock data).")
            return [{"aggregate_key": "mock_key", "count": 10, "avg_value": 5.5}]

    mock_db = MockTimeSeriesDB()

    await mock_db.initialize()
    await mock_db.record_event({"event_type": "test_event", "timestamp": datetime.now().isoformat()})
    await mock_db.query_events("test_event", datetime.now(), datetime.now())
    async for event in mock_db.query_events_generator("test_event", datetime.now(), datetime.now()):
