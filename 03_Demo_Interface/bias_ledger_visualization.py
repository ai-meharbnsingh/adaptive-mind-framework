...
class BiasPatternAnalyzer:
    ...
    def analyze_patterns(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        if len(events) < 2:
            return {"temporal_patterns": {}, "provider_correlations": {},
                    "impact_trends": {}, "predictions": [], "analysis_confidence": 0.0}
    ...
    def _analyze_temporal_patterns(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        return {
            ...
            "daily_variety": {day: len(set(types))
                              for day, types in daily_trends.items()}
        }
    ...
    def _analyze_impact_trends(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        if len(events) < 5:
            return {"trend": "insufficient_data", "trend_slope": 0.0, "current_avg_impact": 0.0,
        ...
        impact_scores = [self._impact_to_score(event.impact_level)
        ...
        if len(rolling_averages) >= 2:
            trend_slope = (rolling_averages[-1] - rolling_averages[0]) / len(rolling_averages)
            if trend_slope > 0.1:
        ...
        current_avg_impact = np.mean(impact_scores[-window_size:]) if len(
            impact_scores) >= window_size and window_size > 0 else (np.mean(impact_scores) if impact_scores else 0.0)
    ...
    def _generate_predictions(self, events: List[BiasEvent]) -> List[Dict[str, Any]]:
        ...
        if len(events) < 10:
            return [{"type": "prediction_status", "prediction": "insufficient_data_for_predictions"}]
        ...
        most_likely_bias = max(bias_frequency.items(), key=lambda x: x[1]) if bias_frequency else ("UNKNOWN", 0)

        # Predict time until next bias event
        time_deltas = []
        for i in range(1, len(recent_events)):
            delta = (recent_events[i].timestamp - recent_events[i - 1].timestamp).total_seconds()
            if delta > 0: time_deltas.append(delta)
...
class LiveBiasLedgerVisualizer:
    ...
    def __init__(self, max_entries: int = 1000):
        ...
        logger.info("LiveBiasLedgerVisualizer initialized with enterprise-grade capabilities")
        ...
        try:
            ...
            for record in reversed(records):
                try:
                    ...
                    context = json.loads(record['context_json']) if isinstance(record['context_json'], str) else record[
                        'context_json']
                    ...
                    logger.error(f"Error reconstructing BiasEvent from DB record {record.get('event_id')}: {e}")
                    continue
            ...
            logger.info(f"Loaded {len(reconstructed_events)} historical bias events from PostgreSQL.")
        except Exception as e:
    ...
    async def add_bias_event(self, event: BiasEvent) -> str:
        ...
        try:
            ...
            logger.info(f"Added bias event {event.id} with learning weight {learning_weight:.3f}")
            return event.id
    ...
    async def get_bias_score_trend(self, session_id: Optional[str] = None,
        ...
        try:
            ...
            for i in range(0, time_window_minutes, interval_minutes):
                ...
                interval_events = [
                    entry.event for entry in self.bias_ledger
                ...
                if interval_events:
                    avg_bias = np.mean([
                        self.pattern_analyzer._impact_to_score(event.impact_level) / 5.0
                        for event in interval_events
                    ])
                else:
                    avg_bias = np.random.uniform(0.05, 0.15)  # Mock low bias when no events
            ...
            return {"labels": labels, "data": data, "chart_type": "line", "title": "Bias Score Trend (Mock)"}
    ...
    def _calculate_adaptation_score(self, event: BiasEvent) -> float:
        ...
        if event.resolution_time and event.timestamp:
            resolution_minutes = (event.resolution_time - event.timestamp).total_seconds() / 60
    ...
    def _calculate_influence_radius(self, event: BiasEvent) -> Dict[str, float]:
        ...
        time_distance = min(abs(current_hour - event_hour), 24 - abs(current_hour - event_hour))
    ...
    def _update_metrics(self, event: BiasEvent):
        ...
        if event.resolution_time and event.timestamp:
            resolution_seconds = (event.resolution_time - event.timestamp).total_seconds()
    ...
    def _generate_provider_heatmap(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        provider_data = defaultdict(lambda: {"count": 0, "severity_sum": 0, "types": set()})

        for event in events:
            provider_data[event.provider]["count"] += 1
            provider_data[event.provider]["severity_sum"] += self.pattern_analyzer._impact_to_score(event.impact_level)
    ...
    def _generate_impact_distribution(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        total_events = len(events)
        percentages = {level: (count / total_events * 100) if total_events > 0 else 0
    ...
    def _generate_learning_progression(self) -> Dict[str, Any]:
        ...
        if not self.bias_ledger:
            return {"progression": [], "trend": "no_data", "learning_efficiency": 0.0}
        ...
        for i in range(window_size, len(ledger_list) + 1, 5):
            ...
            avg_learning_weight = np.mean([entry.learning_weight for entry in window]) if window else 0.0
            avg_adaptation_score = np.mean([entry.adaptation_score for entry in window]) if window else 0.0

            progression.append({
                "event_index": i,
                "avg_learning_weight": avg_learning_weight,
                "avg_adaptation_score": avg_adaptation_score,
                "timestamp": window[-1].event.timestamp.isoformat() if window else datetime.now(
        ...
        if len(progression) >= 2:
            ...
            trend = "improving" if recent_adaptation > early_adaptation else "declining" if recent_adaptation < early_adaptation else "stable"
    ...
    def _generate_live_predictions(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        if critical_events:
            ...
            time_since_critical = (datetime.now(timezone.utc) - last_critical).total_seconds() / 3600
    ...
    def _calculate_learning_acceleration(self) -> float:
        ...
        historical_avg = np.mean(historical_weights) if historical_weights else recent_avg
    ...
    def _calculate_system_health_score(self) -> float:
        ...
        avg_adaptation = np.mean([entry.adaptation_score for entry in recent_entries]) if recent_entries else 0.0

        resolved_events = [entry for entry in recent_entries
                           if entry.event.resolution_time is not None]
        if resolved_events:
            avg_resolution_time = np.mean([
                (entry.event.resolution_time - entry.event.timestamp).total_seconds() / 60
                for entry in resolved_events if entry.event.resolution_time and entry.event.timestamp
            ])
        ...
        learning_efficiency = self._calculate_learning_efficiency()
        health_score = (avg_adaptation * 0.4 + resolution_score * 0.3 + learning_efficiency * 0.3)
        ...
        for bias_type, entries in bias_type_groups.items():
            if len(entries) >= 2:
                ...
                if len(adaptation_scores) >= 2:
                    improvement = (adaptation_scores[-1] - adaptation_scores[0]) / max(adaptation_scores[0], 0.1)
    ...
    def _calculate_cost_trend(self, events: List[BiasEvent]) -> Dict[str, Any]:
        ...
        if len(events) < 5:
            return {"trend": "insufficient_data", "slope": 0.0, "recent_avg_cost": 0.0, "total_cost_impact": 0.0}
        ...
        recent_avg_cost = np.mean([self._estimate_cost_impact(event) for event in sorted_events[-5:]]) if len(
            sorted_events) >= 5 else (
            np.mean([self._estimate_cost_impact(event) for event in sorted_events]) if sorted_events else 0.0)
    ...
    def _generate_antifragile_indicators(self) -> Dict[str, Any]:
        ...
        high_impact_events = [entry for entry in ledger_list
                              if entry.event.impact_level in [BiasImpact.CRITICAL, BiasImpact.HIGH]]
        resilience_score = np.mean(
            [entry.adaptation_score for entry in high_impact_events]) if high_impact_events else 0.5

        antifragile_growth = 0.0
        if len(ledger_list) >= 20:
            early_performance = np.mean([entry.adaptation_score for entry in ledger_list[:10]]) if ledger_list[
                                                                                                   :10] else 0.0
            recent_performance = np.mean([entry.adaptation_score for entry in ledger_list[-10:]]) if ledger_list[
                                                                                                     -10:] else 0.0
            antifragile_growth = (recent_performance - early_performance) / max(early_performance, 0.1)

        antifragile_score = (avg_adaptation * 0.3 +
    ...
    def _calculate_stress_tolerance(self) -> float:
        ...
        ledger_list = list(self.bias_ledger)
        high_stress_events = [
            entry for entry in ledger_list
            if entry.event.impact_level in [BiasImpact.CRITICAL, BiasImpact.HIGH]
        ]
        ...
        stress_adaptation = np.mean(
            [entry.adaptation_score for entry in high_stress_events]) if high_stress_events else 0.0
    ...
    def _calculate_evolutionary_progress(self) -> float:
        ...
        early_adaptation = np.mean([entry.adaptation_score for entry in early_window]) if early_window else 0.0
        recent_adaptation = np.mean([entry.adaptation_score for entry in recent_window]) if recent_window else 0.0
        adaptation_evolution = (recent_adaptation - early_adaptation) / max(early_adaptation, 0.1)

        early_learning = np.mean([entry.learning_weight for entry in early_window]) if early_window else 0.0
        recent_learning = np.mean([entry.learning_weight for entry in recent_window]) if recent_window else 0.0
        learning_evolution = (recent_learning - early_learning) / max(early_learning, 0.1)
...
class BiasLedgerDemoGenerator:
    ...
    def generate_demo_event(self, scenario: str = None) -> BiasEvent:
        ...
        event = BiasEvent(
            ...
            context={
                ...
                "geographical_region": random.choice(["us-east", "eu-west", "asia-pacific"])
            },
            corrective_action="Automatic failover initiated" if random.random() > 0.6 else None,
            resolution_time=datetime.now(timezone.utc) - timedelta(
                minutes=random.randint(1, 30)) if random.random() > 0.4 else None,
    ...
    async def populate_demo_ledger_with_persistence(self, visualizer: LiveBiasLedgerVisualizer, num_events: int = 20):
        """Populate the ledger with demo events, ensuring they are persisted to DB"""
        logger.info(f"Populating demo ledger with {num_events} events (with persistence)")
...
class FrameworkBiasLedgerIntegration:
    ...
    async def handle_provider_failure(self, provider: str, failure_data: Dict[str, Any]):
...
if __name__ == "__main__":
    ...
    async def demo_bias_ledger():
        ...
        try:
            ...
            await integration.handle_provider_failure("openai", {
            ...
            print(f"  Antifragile Score: {antifragile.get('antifragile_score', 0.0):.3f}")
            print(f"  Adaptation Velocity: {antifragile.get('adaptation_velocity', 0.0):.3f}")
            print(f"  Learning Diversity: {antifragile.get('learning_diversity', 0.0):.3f}")
            print(f"  Resilience Score: {antifragile.get('resilience_score', 0.0):.3f}")

            print("\n" + "=" * 80)
            print("✅ BIAS LEDGER DEMONSTRATION COMPLETE")
            print("🎯 Live real-time bias tracking and antifragile learning operational")
