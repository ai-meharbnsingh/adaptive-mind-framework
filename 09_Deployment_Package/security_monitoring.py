...
API_KEY_VALIDATION_TIME = Histogram('api_key_validation_seconds', 'API key validation time')
...
class SecurityMonitor:
    ...
    async def validate_api_key(self, api_key: str, provider: str, source_ip: str) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if self.config.get('deployment_modes', {}).get('hosted_demo', {}).get('security', {}).get(
                    'api_key_validation') == 'mock':
            ...
            elif self.config.get('deployment_modes', {}).get('buyer_evaluation', {}).get('security', {}).get(
    ...
    def _detect_injection_attacks(self, payload: str) -> Optional[str]:
        ...
        for attack_type, patterns in self.threat_patterns.items():
            if attack_type in ['sql_injection', 'xss_attack', 'command_injection', 'path_traversal']:
    ...
    async def _check_rate_limits(self, ip_address: str) -> Optional[Dict[str, Any]]:
        ...
        while (self.request_counts[ip_address] and
               self.request_counts[ip_address][0] < current_time - window_size):
            self.request_counts[ip_address].popleft()

        # Check rate limit
        request_count = len(self.request_counts[ip_address])
        rate_limit = self.config.get('shared', {}).get('rate_limiting', {}).get('requests_per_minute', 100)
    ...
    async def _log_security_event(self, event_type: str, severity: str,
        ...
        """Log security event"""
        event = SecurityEvent(
            id=hashlib.sha256(f"{time.time()}{source_ip}{event_type}".encode()).hexdigest()[:16],
            timestamp=datetime.now(timezone.utc),
        ...
        if self.db_pool:
            try:
                async with self.db_pool.acquire() as conn:
                    ...
                    """, event.id, event.timestamp, event.event_type, event.severity,
            ...
            try:
                ...
                for ip in list(self.request_counts.keys()):
                    while (self.request_counts[ip] and
                           self.request_counts[ip][0] < current_time - 3600):  # 1 hour
                ...
                for ip in self.blocked_ips:
                    if (not self.request_counts.get(ip) or
                            len(self.request_counts[ip]) == 0):
            ...
            try:
                ...
                for ip in list(self.failed_validations.keys()):
                    if self.failed_validations[ip] > 0:
                        ...
                        self.failed_validations[ip] = max(0, self.failed_validations[ip] - 1)
        ...
        while self.running:
            try:
                ...
                while (self.security_events and
                       self.security_events[0].timestamp < cutoff_time):
                ...
                if self.db_pool:
                    try:
                        async with self.db_pool.acquire() as conn:
                            retention_days = self.config.get('data_management', {}).get('retention', {}).get(
                                'audit_logs', '7y')
            ...
            for ip, events in ip_events.items():
                ...
                if high_severity_count >= 3:
                    ...
                    await self._log_security_event(
                        ...
                        metadata={'event_count': len(events), 'high_severity_count': high_severity_count}
                    )
    ...
    async def _detect_coordinated_attacks(self):
        ...
        try:
            ...
            for event in recent_events:
                window = event.timestamp.replace(minute=event.timestamp.minute // 10 * 10, second=0, microsecond=0)
                ...
                for event_type, ips in event_types.items():
                    if len(ips) >= 5:  # 5+ different IPs with same event type
                        await self._log_security_event(
                            ...
                            metadata={'ip_count': len(ips), 'time_window': window.isoformat()}
                        )
    ...
    def get_security_status(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'last_report_time': current_time.replace(minute=0, second=0, microsecond=0).isoformat()
        }
