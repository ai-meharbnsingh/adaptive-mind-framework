...
try:
    from antifragile_framework.database.connection_manager import PostgreSQLConnectionManager
except ImportError:
    try:
...
class ProviderPerformanceTracker:
    ...
    async def _generate_mock_performance_data(self):
        ...
        scenarios = ["customer_service", "fraud_detection", "content_moderation", "financial_analysis",
    ...
    async def _load_historical_performance_records_from_db(self):
        ...
        try:
            ...
            logger.error(f"Error loading historical performance records from database: {e}")
            # Fall back to mock data
            await self._generate_mock_performance_data()

    async def _persist_performance_record_to_db(self, provider_id: str, record: Dict[str, Any]):
        """
        ...
        try:
            ...
            logger.error(f"Error persisting performance record for {provider_id} to database: {e}")

    async def record_request_outcome(self, provider_id: str, outcome_data: Dict[str, Any]):
        ...
        logger.debug(f"Recorded performance data for {provider_id}: success={outcome_data.get('success')}")
    ...
    async def get_provider_metrics(self, provider_id: str) -> ProviderMetrics:
        ...
        # Check cache
        if (provider_id in self.last_cache_update and
                current_time - self.last_cache_update[provider_id] < self.cache_ttl and
                provider_id in self.metrics_cache):
    ...
    def _calculate_provider_metrics(self, provider_id: str) -> ProviderMetrics:
        ...
        response_times = [record["response_time_ms"] for record in history if record["success"]]
        ...
        quality_scores = [record["quality_score"] for record in history if record["success"]]
    ...
    def _calculate_learning_compatibility(self, history: List[Dict[str, Any]]) -> float:
        ...
        for i in range(0, len(history) - window_size + 1, window_size // 2 or 1):
            ...
            avg_quality = statistics.mean(valid_quality_scores) if valid_quality_scores else 0.0
    ...
    def _calculate_stress_tolerance(self, history: List[Dict[str, Any]]) -> float:
        ...
        normal_performance = statistics.mean(normal_performance_records) if normal_performance_records else 1.0
        high_load_performance = statistics.mean(
            high_load_performance_records) if high_load_performance_records else normal_performance
    ...
    def _calculate_hourly_performance(self, history: List[Dict[str, Any]]) -> Dict[int, float]:
        ...
        for hour, successes in hourly_data.items():
            if successes:
                hourly_performance[hour] = sum(1 for s in successes if s) / len(successes)

        return hourly_performance

    def _calculate_daily_trends(self, history: List[Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _calculate_scenario_performance(self, history: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for scenario, successes in scenario_data.items():
            if successes:
                scenario_performance[scenario] = sum(1 for s in successes if s) / len(successes)

        return scenario_performance

    def _calculate_load_performance(self, history: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        for load_level, successes in load_data.items():
            if successes:
                load_performance[load_level] = sum(1 for s in successes if s) / len(successes)
...
class InteractiveProviderRankingSystem:
    ...
    async def register_provider(self, provider_id: str, initial_status: ProviderStatus = ProviderStatus.ACTIVE):
        ...
        logger.info(f"Provider registered: {provider_id} with status {initial_status.value}")
        await self._update_rankings()

    async def record_provider_performance(self, provider_id: str, outcome_data: Dict[str, Any]):
    ...
    async def _update_rankings(self):
        ...
        try:
            ...
            for provider_id in list(self.active_providers):
                metrics = await self.performance_tracker.get_provider_metrics(provider_id)
            ...
            for rank, (provider_id, (metrics, weighted_score)) in enumerate(sorted_providers, 1):
                prev_ranking = next((r for r in self.current_rankings if r.provider_id == provider_id), None)
                ...
                strengths, weaknesses = self._analyze_provider_strengths_weaknesses(metrics)
                trending = self._determine_trending_direction(provider_id, metrics)
                recommendation = self._generate_provider_recommendation(metrics, status, rank)
    ...
    def _calculate_weighted_score(self, metrics: ProviderMetrics) -> float:
        ...
        metric_values = {
            ...
            RankingMetric.RESPONSE_TIME: max(0.0, 1.0 - (metrics.response_time_ms / 5000.0))
        }
    ...
    def _calculate_detailed_scores(self, metrics: ProviderMetrics) -> Dict[RankingMetric, float]:
    ...
    def _analyze_provider_strengths_weaknesses(self, metrics: ProviderMetrics) -> Tuple[List[str], List[str]]:
        ...
        metric_analysis = {
            "reliability": (metrics.reliability_score, "Excellent reliability"),
            "performance": (metrics.performance_score, "Fast response times"),
            "cost_efficiency": (metrics.cost_efficiency_score, "Cost-effective operations"),
            "quality": (metrics.quality_score, "High output quality"),
            "availability": (metrics.availability_score, "Strong uptime record"),
            "bias_resistance": (metrics.bias_resistance_score, "Consistent across scenarios"),
            "learning_compatibility": (metrics.learning_compatibility_score, "Adapts well to learning"),
            "stress_tolerance": (metrics.stress_tolerance, "Handles high load well")
        }
        ...
        for metric_name, (score, strength_desc) in metric_analysis.items():
            if score >= high_threshold:
                ...
                weakness_desc = strength_desc.replace("Excellent", "Poor").replace("Fast", "Slow").replace("High",
                                                                                                           "Low").replace(
                    "Strong", "Weak").replace("Consistent", "Inconsistent").replace("well", "poorly")
                weaknesses.append(weakness_desc)

        return strengths, weaknesses

    def _determine_trending_direction(self, provider_id: str, metrics: ProviderMetrics) -> str:
    ...
    def _generate_provider_recommendation(self, metrics: ProviderMetrics, status: ProviderStatus, rank: int) -> str:
    ...
    async def _check_ranking_update_trigger(self):
        ...
        if not hasattr(self, '_last_ranking_update'):
            self._last_ranking_update = current_time - timedelta(seconds=self.ranking_update_interval)
    ...
    async def update_provider_status(self, provider_id: str, new_status: ProviderStatus, reason: str = ""):
        ...
        logger.info(f"Provider {provider_id} status changed: {old_status.value} -> {new_status.value}")
        if reason:
    ...
    async def get_ranking_analytics(self) -> Dict[str, Any]:
        ...
        analytics = {
            "overview": {
                ...
                "active_providers": len([p for p, s in self.provider_statuses.items()
                                         if s == ProviderStatus.ACTIVE]),
                "degraded_providers": len([p for p, s in self.provider_statuses.items()
                                           if s == ProviderStatus.DEGRADED]),
                "ranking_stability": 0.8,  # Simplified for demo
                "last_updated": datetime.now(timezone.utc).isoformat()
            },
            "performance_trends": self._analyze_performance_trends(),
            "ranking_changes": {"recent_changes": [], "biggest_movers": {"up": None, "down": None},
    ...
    def _analyze_performance_trends(self) -> Dict[str, Any]:
        ...
        active_providers = [r for r in self.current_rankings if r.status == ProviderStatus.ACTIVE]
        ...
        for metric in RankingMetric:
            metric_values = [r.metric_scores.get(metric, 0) for r in self.current_rankings]
            if metric_values:
        ...
        for ranking in self.current_rankings:
            provider_insights = {
                ...
                "key_metrics": {
                    ...
                    "stability": ranking.rank_stability,
                    "top_strength": {"metric": "reliability", "score": 0.95} if ranking.strengths else None,
                    "main_weakness": {"metric": "cost_efficiency", "score": 0.3} if ranking.weaknesses else None
                },
    ...
    def _calculate_system_antifragile_metrics(self) -> Dict[str, Any]:
        ...
        if not self.current_rankings:
            return {"system_resilience": 0.0, "diversity_score": 0.0, "adaptation_capability": 0.0,
                    "provider_count": 0, "active_provider_count": 0, "overall_antifragile_score": 0.0}

        active_providers = [r for r in self.current_rankings if r.status == ProviderStatus.ACTIVE]
        system_resilience = len(active_providers) / len(self.current_rankings) if self.current_rankings else 0.0

        diversity_score = 0.8  # Simplified for demo
        adaptation_capability = 0.7  # Simplified for demo
        overall_antifragile_score = (system_resilience + diversity_score + adaptation_capability) / 3.0
    ...
    def _generate_system_recommendations(self) -> List[str]:
        ...
        if len(self.current_rankings) < 3:
            recommendations.append("Consider adding more providers for better resilience")

        avg_score = statistics.mean([r.overall_score for r in self.current_rankings]) if self.current_rankings else 0.0
        if avg_score < 0.6:
            recommendations.append("Overall provider performance is below optimal - consider optimization")
    ...
    async def _broadcast_update(self, update_data: Dict[str, Any]):
        ...
        for sub_id, queue in self.subscribers.items():
            try:
                await asyncio.wait_for(queue.put(json.dumps(update_data, default=str)), timeout=1.0)
...
class ProviderRankingDemoGenerator:
    ...
    def __init__(self):
        ...
        self.scenarios = ["customer_service", "fraud_detection", "content_moderation", "analysis", "translation"]
        self.load_levels = ["low", "normal", "high"]

    async def simulate_provider_activity(self, ranking_system: InteractiveProviderRankingSystem,
                                         duration_minutes: int = 5):
        """Simulate realistic provider activity for demo purposes"""
        logger.info(f"Starting provider activity simulation for {duration_minutes} minutes")
        ...
        provider_profiles = {
            "openai": {"base_success_rate": 0.95, "base_response_time": 1200, "base_cost": 0.02},
            "anthropic": {"base_success_rate": 0.97, "base_response_time": 1500, "base_cost": 0.025},
            "google": {"base_success_rate": 0.92, "base_response_time": 800, "base_cost": 0.015},
            "azure": {"base_success_rate": 0.94, "base_response_time": 1100, "base_cost": 0.018},
            "cohere": {"base_success_rate": 0.90, "base_response_time": 1000, "base_cost": 0.012}
        }
        ...
        while datetime.now(timezone.utc) < end_time:
            ...
            if random.random() < 0.02:  # 2% chance
                ...
                if ranking_system.provider_statuses.get(provider) == ProviderStatus.DEGRADED:
        ...
        logger.info(f"Provider activity simulation complete: {request_count} requests processed")
...
if __name__ == "__main__":
    ...
    async def demo_provider_ranking_system():
        ...
        try:
            ...
            update_queue = await ranking_system.subscribe_to_ranking_updates(subscription_id)

            # Start simulation task
            simulation_task = asyncio.create_task(
                demo_generator.simulate_provider_activity(ranking_system, duration_minutes=0.5)
            )
            ...
            while update_count < 3 and not simulation_task.done():
                try:
                    update_json_str = await asyncio.wait_for(update_queue.get(), timeout=5.0)
                    update = json.loads(update_json_str)
                    update_count += 1
                    print(f"📊 Update {update_count}: {update['type']}")

                    if update['type'] == 'rankings_updated':
                        print(f"   New rankings for {len(update['rankings'])} providers")
                        ...
                        print(f"   Provider {update['provider_id']} status changed to {update['new_status']}")
            ...
            for ranking in rankings:
                ...
                print(f"      Score: {ranking.overall_score:.3f} | Status: {ranking.status.value}")
            ...
            print(f"  Database Available: {ranking_system.performance_tracker.db_available}")

            print(f"\n🛡️  ANTIFRAGILE METRICS:")
            antifragile = analytics["antifragile_metrics"]
            print(f"  System Resilience: {antifragile['system_resilience']:.3f}")
            print(f"  Overall Antifragile Score: {antifragile['overall_antifragile_score']:.3f}")
            ...
            if ranking_system.performance_tracker.db_available and ranking_system.performance_tracker.db_manager:
