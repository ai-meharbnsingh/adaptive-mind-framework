...
from antifragile_framework.core.online_learning_subscriber import OnlineLearningSubscriber
...
DEFAULT_PROVIDER_CONFIGS = {
    "openai": {"api_keys": get_api_keys_from_env("OPENAI_API_KEY"), "resource_config": {},
               "circuit_breaker_config": {}},
    "google_gemini": {"api_keys": get_api_keys_from_env("GEMINI_API_KEY"), "resource_config": {},
                      "circuit_breaker_config": {}},
    "anthropic": {"api_keys": get_api_keys_from_env("ANTHROPIC_API_KEY"), "resource_config": {},
                  "circuit_breaker_config": {}}
}
...
class ChatCompletionRequest(BaseModel):
    model_priority_map: Dict[str, List[str]] = Field(...,
                                                     description="A dictionary mapping provider names to a prioritized list of their models. The order of providers in this map serves as a fallback if no dynamic ranking is available.",
                                                     examples=[{"openai": ["gpt-4o", "gpt-4-turbo"],
                                                                "google_gemini": ["gemini-1.5-flash-latest"],
                                                                "anthropic": ["claude-3-5-sonnet-20240620"]}])
    messages: List[ChatMessage]
    preferred_provider: Optional[str] = Field(
        None,
        description="Optional: The user's preferred provider to attempt first. If specified, the system will prioritize this provider before falling back to dynamic ranking.",
        examples=["openai", "anthropic"]
    )
    max_estimated_cost_usd: Optional[float] = Field(
        None,
        description="Optional: The maximum estimated cost (in USD) for this single API call. If a model's estimated cost for the current request exceeds this cap, it will be skipped.",
        examples=[0.01, 0.05]
    )
...
async def lifespan(app: FastAPI):
    """
    Manages the application's lifespan, initializing and wiring together all core components.
    """
    core_logger.log(UniversalEventSchema(event_type="api.startup.begin", event_source="framework_api.lifespan",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
                                         payload={"message": "Initializing Adaptive Mind components..."}))

    is_perf_mode = os.getenv("PERFORMANCE_TEST_MODE", "False").lower() == "true"
    if is_perf_mode:
        print("[OK] Lifespan: PERFORMANCE_TEST_MODE is 'True'. Initializing with mocked providers.")
        core_logger.log(UniversalEventSchema(event_type="api.startup.mode", event_source="framework_api.lifespan",
                                             timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="WARNING",
                                             payload={"message": "PERFORMANCE_TEST_MODE is active."}))
    else:
        print("[INFO] Lifespan: PERFORMANCE_TEST_MODE is not set. Initializing in production mode.")

    try:
        provider_profiles = load_provider_profiles()
    except (FileNotFoundError, ValueError) as e:
        core_logger.log(UniversalEventSchema(event_type="api.startup.failure", event_source="framework_api.lifespan",
                                             timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="CRITICAL",
                                             payload={"error": f"Failed to load provider profiles: {e}"}))
        sys.exit(f"CRITICAL ERROR: Could not start API due to invalid provider profiles configuration. Error: {e}")
    ...
    core_logger.log(UniversalEventSchema(event_type="api.startup.wiring", event_source="framework_api.lifespan",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
    ...
    core_logger.log(UniversalEventSchema(event_type="api.startup.end", event_source="framework_api.lifespan",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
                                         payload={"message": "Adaptive Mind API initialization complete."}))
    yield
    core_logger.log(UniversalEventSchema(event_type="api.shutdown", event_source="framework_api.lifespan",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
                                         payload={"message": "Adaptive Mind API shutting down."}))
    event_bus.shutdown()


app = FastAPI(title="Adaptive Mind Resilience Framework API", version="1.9.0", # Version Bump
              description="Exposes the core functionality of the Adaptive Mind framework.", lifespan=lifespan)


@app.exception_handler(AllProvidersFailedError)
async def all_providers_failed_exception_handler(request: Request, exc: AllProvidersFailedError):
    request_id = getattr(request.state, 'request_id', "N/A")
    core_logger.log(UniversalEventSchema(event_type=event_topics.API_SERVICE_UNAVAILABLE,
                                         event_source="framework_api.exception_handler",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="ERROR",
                                         payload={"request_id": request_id, "error": str(exc),
                                                  "client_host": request.client.host if request.client else "N/A"}))
    return JSONResponse(status_code=503, content={"detail": "All underlying AI providers are currently unavailable."},
...
async def generic_exception_handler(request: Request, exc: Exception):
    request_id = getattr(request.state, 'request_id', "N/A")
    core_logger.log(UniversalEventSchema(event_type=event_topics.API_UNHANDLED_ERROR,
                                         event_source="framework_api.exception_handler",
                                         timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="CRITICAL",
...
async def logging_and_request_id_middleware(request: Request, call_next):
    ...
    core_logger.log(
        UniversalEventSchema(event_type=event_topics.API_REQUEST_START, event_source="framework_api.middleware",
                             timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
    ...
    core_logger.log(
        UniversalEventSchema(event_type=event_topics.API_REQUEST_END, event_source="framework_api.middleware",
                             timestamp_utc=datetime.now(timezone.utc).isoformat(), severity="INFO",
                             payload={"request_id": request_id, "status_code": response.status_code,
...
@app.post("/v1/chat/completions", response_model=CompletionResponse,
          responses={503: {"model": ErrorDetail}, 500: {"model": ErrorDetail}}, tags=["Core Functionality"])
...
async def get_provider_rankings(request: Request):
    """
    Returns the current real-time performance scores and rankings of all providers.
    """
...
if __name__ == "__main__":
    ...
    uvicorn.run("antifragile_framework.api.framework_api:app", host="127.0.0.1", port=8000, reload=True,
                log_level="info")