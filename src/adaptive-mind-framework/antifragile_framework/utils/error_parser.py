...
class ErrorParser:
    def __init__(self):
        ...
        self.model_error_keywords = [
            ...
            "invalid model", "model does not exist", "model is deprecated",
    ...
    def classify_error(self, exception: Exception, provider_name: str) -> ErrorDetails:
        ...
        if OPENAI_AVAILABLE and isinstance(exception, openai.BadRequestError):
            ...
            if error_details.get('error_code') == 'content_policy_violation':
                return ErrorDetails(category=ErrorCategory.CONTENT_POLICY, is_retriable=True, provider=provider_name,
                                    **error_details)
            ...
            if self._is_model_error_by_message(error_details.get('error_message', '')):
                return ErrorDetails(category=ErrorCategory.MODEL_ISSUE, is_retriable=False, provider=provider_name,
                                    **error_details)
            return ErrorDetails(category=ErrorCategory.FATAL, is_retriable=False, provider=provider_name,
                                **error_details)
        ...
        if ANTHROPIC_AVAILABLE and isinstance(exception, anthropic.BadRequestError):
            ...
            if error_type == 'invalid_request_error' and 'safety' in error_details.get('error_message', '').lower():
                return ErrorDetails(category=ErrorCategory.CONTENT_POLICY, is_retriable=True, provider=provider_name,
                                    **error_details)
            if self._is_model_error_by_message(error_details.get('error_message', '')):
                return ErrorDetails(category=ErrorCategory.MODEL_ISSUE, is_retriable=False, provider=provider_name,
        ...
        for exc_type in type(exception).__mro__:
            if exc_type in self.EXCEPTION_MAP:
                if self.EXCEPTION_MAP[exc_type] == ErrorCategory.FATAL and self._is_model_error_by_message(
                        error_message):
                    return ErrorDetails(
                        category=ErrorCategory.MODEL_ISSUE,
                        is_retriable=False,
                ...
                retry_after = self._extract_retry_after(exception) if category == ErrorCategory.TRANSIENT else None
                return ErrorDetails(
                    category=category,
        ...
        return ErrorDetails(category=ErrorCategory.UNKNOWN, is_retriable=False, provider=provider_name,
                            error_message=error_message)

    def _extract_openai_error_details(self, exception: openai.APIError) -> Dict[str, Any]:
        try:
            body = getattr(exception, 'body', {}) or {}
            error = body.get('error', {}) or {}
            return {'error_code': error.get('code'), 'error_message': error.get('message'),
                    'metadata': {'type': error.get('type'), 'param': error.get('param')}}
        except Exception:
            return {'error_message': str(exception)}

    def _extract_anthropic_error_details(self, exception: anthropic.APIError) -> Dict[str, Any]:
        try:
            if hasattr(exception, 'response'):
                error_data = exception.response.json()
                if isinstance(error_data, dict) and 'error' in error_data:
                    err = error_data['error']
                    return {'error_code': err.get('type'), 'error_message': err.get('message')}
        except Exception:
            pass
        return {'error_message': str(exception)}
