...
TELEMETRY_PATH = CURRENT_DIR.parent.parent.parent.parent / "telemetry"  # Points to 01_Framework_Core/telemetry/
...
try:
    ...
    logging.critical(f"CRITICAL ERROR: Failed to import core dependencies for LearningEngine: {e}. "
                     f"LearningEngine will not function correctly.", exc_info=True)
    ...
    class MockCoreLogger:
        def log(self, event): logging.info(f"MockCoreLogger: {event.get('event_type')}")
    ...
    class BiasLedgerEntry:
        ...
        def model_validate(cls, data): return BiasLedgerEntry()
...
class LearningEngine:
    ...
    async def get_raw_bias_ledger_entries(self,  # Changed to async for proper DB interaction
                                          start_time: datetime,
                                          end_time: datetime,
                                          batch_size: int = 1000) -> Iterator[Dict[str, Any]]:  # Yield raw dicts first
        """
        ...
        async for raw_event_dict in self.db_interface.query_events_generator(  # Use async for generator
    ...
    async def analyze_provider_performance(self,  # Changed to async
                                           start_time: datetime,
        ...
        async for event_payload_dict in self.get_raw_bias_ledger_entries(start_time, end_time):  # Await the generator
            try:
                ...
                if ledger_entry.outcome == 'SUCCESS' or ledger_entry.outcome == 'MITIGATED_SUCCESS':
                    provider_model_metrics['successful_requests'] += 1
                    provider_model_metrics['total_latency_ms'] += ledger_entry.total_latency_ms
                else:
                    error_type = 'unknown_failure'
                    if ledger_entry.resilience_events:
                        for event in reversed(ledger_entry.resilience_events):  # Iterate through lifecycle events
                            event_type = event.get('event_type')
                            payload_inner = event.get('payload', {})
                            if event_type == event_topics.API_CALL_FAILURE and payload_inner.get('error_type'):
                ...
                for event in ledger_entry.resilience_events:  # Iterate through lifecycle events
                    if event.get('event_type') in [event_topics.PROVIDER_FAILOVER, event_topics.MODEL_FAILOVER,
                ...
                if ledger_entry.resilience_score is not None:
                    provider_model_metrics['resilience_scores_sum'] += ledger_entry.resilience_score
        ...
        for provider_name, models_data in aggregated_data.items():
            for model_name, metrics in models_data.items():
                ...
                circuit_breaker_trip_rate = metrics[
                                                'circuit_breaker_tripped_count'] / total_requests if total_requests > 0 else 0.0
                avg_resilience_score = metrics['resilience_scores_sum'] / metrics['resilience_score_count'] if metrics[
                                                                                                                   'resilience_score_count'] > 0 else 1.0  # Default to 1.0 if no scores recorded
...
async def main():
    ...
    class MockTimeSeriesDB(TimeSeriesDBInterface):
        ...
        async def query_events_generator(self, event_type, start_time, end_time, batch_size):
            if event_type == event_topics.BIAS_LOG_ENTRY_CREATED:
                for i in range(15):  # Generate 15 mock events
                    ...
                    resilience_score = 1.0 if outcome == "SUCCESS" else (0.5 if i % 3 == 1 else 0.2)
                    latency = random.uniform(100, 500) if outcome == "SUCCESS" else random.uniform(1000, 3000)

                    event_payload = {
                        "request_id": str(uuid.uuid4()),
                        "timestamp_utc": (datetime.now(timezone.utc) - timedelta(minutes=15 - i)).isoformat(),
                        "schema_version": 4,
                        "initial_prompt_hash": "abc", "initial_prompt_preview": "...",
                        "outcome": outcome,
                        "total_latency_ms": latency,
                        "total_api_calls": 1,
                        "final_provider": provider,
                        "final_model": model,
                        "resilience_events": [{"event_type": event_topics.API_CALL_FAILURE, "payload": {
                            "error_type": "rate_limit"}} if outcome == "FAILURE" else {}],
                        "mitigation_attempted": (i % 3 == 1),
                        "mitigation_succeeded": (outcome == "MITIGATED_SUCCESS"),
                        "resilience_score": resilience_score,
                        "preferred_provider_requested": None, "initial_selection_mode": "VALUE_DRIVEN",
                        "failover_reason": None, "cost_cap_enforced": False, "cost_cap_skip_reason": None,
                        "input_tokens": 50, "output_tokens": 100, "estimated_cost_usd": 0.001
                    }
