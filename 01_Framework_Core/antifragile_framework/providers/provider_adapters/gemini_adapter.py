    ...
    async def agenerate_completion(
        ...
        try:
            ...
            if not user_assistant_messages_raw:
                ...
                return CompletionResponse(success=False, latency_ms=framework_latency,
            ...
            print(f"🐛 DEBUG: api_call_start = {api_call_start}")
            print(f"🐛 DEBUG: preprocessing = {(api_call_start - framework_start) * 1000:.2f}ms")
            ...
            print(f"🐛 DEBUG: api_call_end = {api_call_end}")
            print(f"🐛 DEBUG: api_call_time = {(api_call_end - api_call_start) * 1000:.2f}ms")
            ...
            if not response.candidates or not hasattr(response, 'text') or not response.text:
                ...
                framework_latency = ((api_call_start - framework_start) +
                                     (framework_end - api_call_end)) * 1000
                print(f"🐛 DEBUG: framework_latency = {framework_latency:.2f}ms")
                print(f"🐛 DEBUG: BREAKDOWN:")
                print(f"🐛 DEBUG:   (api_call_start - framework_start) = {(api_call_start - framework_start) * 1000:.2f}ms")
                print(f"🐛 DEBUG:   (framework_end - api_call_end) = {(framework_end - api_call_end) * 1000:.2f}ms")
                ...
                return CompletionResponse(success=False, content=None, model_used=model_to_use,
                                          latency_ms=framework_latency, error_message=error_msg,
            ...
            print(f"🐛 DEBUG: BREAKDOWN:")
            print(f"🐛 DEBUG:   preprocessing = {(api_call_start - framework_start) * 1000:.2f}ms")
            print(f"🐛 DEBUG:   postprocessing = {(framework_end - api_call_end) * 1000:.2f}ms")
            print(f"🐛 DEBUG:   SUM = {framework_latency:.2f}ms")

            return CompletionResponse(success=True, content=content, model_used=model_to_use, usage=usage,
                                      latency_ms=framework_latency, error_message=None, metadata=metadata)
