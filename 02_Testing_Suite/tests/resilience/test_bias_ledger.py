...
def mock_provider_profiles():
    ...
    return ProviderProfiles(
        ...
        profiles={
            "openai": {
                "_default": CostProfile(input_cpm=Decimal("0.50"), output_cpm=Decimal("1.50")),
                "gpt-4o": CostProfile(input_cpm=Decimal("5.00"), output_cpm=Decimal("15.00"))
            },
            "anthropic": {
                # No default, to test missing profile case
                "claude-3-opus-20240229": CostProfile(input_cpm=Decimal("15.00"), output_cpm=Decimal("75.00"))
            }
        }
    )
...
def test_bias_ledger_initialization(mock_event_bus, mock_provider_profiles):
    ledger = BiasLedger(event_bus=mock_event_bus, provider_profiles=mock_provider_profiles)
...
def test_log_successful_request(mock_event_bus, mock_logger, basic_context):
    ...
    response = CompletionResponse(
        ...
        model_used="gpt-4o", latency_ms=100.0,
    ...
    assert payload['initial_selection_mode'] == "VALUE_DRIVEN" # NEW: Assert the new field
...
def test_log_mitigated_success_request(mock_event_bus, mock_logger, basic_context):
    ...
    response = CompletionResponse(success=True, content="Successful response", model_used="gpt-4o", latency_ms=100.0)
...
def test_cost_calculation_with_specific_profile(mock_event_bus, mock_logger, basic_context, mock_provider_profiles):
    """Verify correct cost calculation using a specific model's profile."""
    ledger = BiasLedger(event_bus=mock_event_bus, provider_profiles=mock_provider_profiles)
    response = CompletionResponse(
        success=True, content="Response", model_used="gpt-4o", latency_ms=100.0,
        metadata={"provider_name": "openai"},
...
def test_cost_calculation_uses_default_profile(mock_event_bus, mock_logger, basic_context, mock_provider_profiles):
    """Verify fallback to the provider's default profile when a specific model profile is missing."""
    ledger = BiasLedger(event_bus=mock_event_bus, provider_profiles=mock_provider_profiles)
    response = CompletionResponse(
        success=True, content="Response", model_used="gpt-4-turbo",  # This model is not in our mock profile
        latency_ms=120.0,
        metadata={"provider_name": "openai"},
...
def test_cost_calculation_handles_missing_provider_profile(mock_event_bus, mock_logger, basic_context,
                                                           mock_provider_profiles):
    """Verify cost is None when the provider has no profile and no default."""
    ledger = BiasLedger(event_bus=mock_event_bus, provider_profiles=mock_provider_profiles)
    response = CompletionResponse(
        success=True, content="Response", model_used="claude-3-sonnet",  # Not in profile
        latency_ms=130.0,
...
def test_no_cost_calculated_if_usage_data_is_missing(mock_event_bus, mock_logger, basic_context,
                                                     mock_provider_profiles):
